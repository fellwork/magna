...
//! a [`fw_graph_config::Preset`].

use clap::Parser;
use fw_graph_config::{JwtConfig, PoolConfig, Preset};

/// fw-graph — GraphQL-from-Postgres server.
#[derive(Parser, Debug)]
#[command(name = "fw-graph", version, about)]
pub struct Config {
...
impl Config {
    /// Convert CLI config into a [`Preset`] suitable for all fw-graph components.
