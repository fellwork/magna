//! magna — top-level binary that wires all 9 magna crates together.
//!
//! Startup sequence:
...
use config::Config;
use magna_build::{build_schema, gather};
use magna_dataplan::PgResourceRegistry;
use magna_introspect::IntrospectionCache;
use magna_serv::{build_router, AppState};
use magna_subscriptions::PgSubscriptionManager;
...
async fn main() {
    ...
    info!(port = config.port, "magna starting");
    ...
    loop {
        match rx.recv().await {
            Ok(target) => {
                ...
                // Re-gather.
                let registry = PgResourceRegistry::from_introspection(&introspection);
                let preset = magna_config::Preset::default();
