//! Integration tests for magna-build against a real Postgres database.
//!
//! Requires a running local Supabase:
//!   DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:54322/postgres
//!
//! Run with: cargo test -p magna-build --test integration
//! (These tests are NOT ignored — they run if the DB is reachable.)

use magna_build::{gather, build_schema, BehaviorSet, GatherOutput, ResourceKind};
use magna_dataplan::PgResourceRegistry;
use magna_introspect::{introspect, IntrospectionResult};
...
async fn setup() -> Option<(sqlx::PgPool, IntrospectionResult, GatherOutput)> {
    ...
    let preset = magna_config::Preset {
        pg_schemas: vec!["public".into()],
        ..magna_config::Preset::default()
    };
...
async fn test_introspect_local_supabase() {
    ...
    let pk_count = result
        .constraints
        .iter()
        .filter(|c| c.kind == magna_introspect::PgConstraintKind::PrimaryKey)
        .count();
...
async fn test_allx_query_returns_rows() {
    ...
    let field_name = magna_build::naming::all_query_field_name(&resource_name.name);
    ...
    let req_conn = magna_build::RequestConnection::new(&pool, &claims)
        .await
...
async fn test_by_pk_query_no_crash() {
    ...
    let field_name = magna_build::naming::by_pk_query_field_name(&resource.name);
    let pk_arg = magna_build::naming::to_camel_case(&resource.primary_key[0]);
    let query_str = format!(
