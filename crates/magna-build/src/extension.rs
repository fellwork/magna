...
//! trait to add domain-specific fields without modifying magna-build.
//!
//! # Lifecycle
//!
        ...
        self.builder.take().unwrap_or_else(|| {
            panic!(
                "ExtensionContext::{caller} called with no builder — \
                 internal invariant violated in magna-build"
            )
        })
    }
}
        ...
        current_builder = Some(ctx.builder.take().unwrap_or_else(|| {
            panic!(
                "extension '{}' left ExtensionContext.builder == None — \
                 this is a bug in magna-build's extension wiring",
                ext.name()
            )
        }));
    }
