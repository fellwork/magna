...
use fw_graph_types::{PgRow, PgTypeOid, PgValue};
...
/// fw-graph-dataplan registers built-in codecs.
/// fw-graph-config plugins can register custom codecs.
pub struct PgCodec {
    pub oid: PgTypeOid,
    pub name: &'static str,
...
///
/// This is the bridge between fw-graph-sql's `(String, Vec<PgValue>)` output
