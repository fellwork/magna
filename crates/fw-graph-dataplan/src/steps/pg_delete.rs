//! PgDeleteStep — builds DELETE ... WHERE ... RETURNING SQL, executes,
//! and returns the deleted rows.

use fw_graph_core::{ExecutableStep, ExecutionContext, StepFingerprint, StepInputs, StepOutput};
use fw_graph_sql::SqlFragment;
use fw_graph_types::{FwGraphError, PgValue, StepId};
use sqlx::PgPool;
use std::any::TypeId;
use std::collections::hash_map::DefaultHasher;
use std::hash::{Hash, Hasher};
use std::sync::Arc;
use tracing::debug;

use crate::codec;

/// A condition for WHERE clauses.
#[derive(Debug, Clone)]
pub struct WhereCondition {
    pub column: String,
    pub value: PgValue,
}

/// Executes a DELETE query against a Postgres table with RETURNING support.
pub struct PgDeleteStep {
    id: StepId,
    pool: PgPool,
    schema: String,
    table: String,

    /// WHERE conditions: column = value pairs (combined with AND).
    conditions: Vec<WhereCondition>,

    /// Columns to return via RETURNING clause.
    /// If empty, RETURNING * is used.
    returning: Vec<String>,

    deps: Vec<StepId>,
}

impl PgDeleteStep {
    /// Create a new PgDeleteStep.
    pub fn new(
        id: StepId,
        pool: PgPool,
        schema: impl Into<String>,
        table: impl Into<String>,
    ) -> Self {
        Self {
            id,
            pool,
            schema: schema.into(),
            table: table.into(),
            conditions: Vec::new(),
            returning: Vec::new(),
            deps: Vec::new(),
        }
    }

    /// Add a WHERE condition (equality check).
    pub fn where_eq(mut self, column: impl Into<String>, value: PgValue) -> Self {
        self.conditions.push(WhereCondition {
            column: column.into(),
            value,
        });
        self
    }

    /// Set the RETURNING columns. If not called, RETURNING * is used.
    pub fn returning(mut self, cols: Vec<String>) -> Self {
        self.returning = cols;
        self
    }

    /// Add an explicit dependency.
    pub fn add_dep(&mut self, dep: StepId) {
        if !self.deps.contains(&dep) {
            self.deps.push(dep);
        }
    }

    fn compute_config_hash(&self) -> u64 {
        let mut h = DefaultHasher::new();
        self.schema.hash(&mut h);
        self.table.hash(&mut h);
        for wc in &self.conditions {
            wc.column.hash(&mut h);
        }
        self.returning.hash(&mut h);
        h.finish()
    }

    /// Build the DELETE SQL and parameter list.
    fn build_sql(&self) -> (String, Vec<PgValue>) {
        let mut frag = SqlFragment::raw("DELETE FROM ")
            .append(SqlFragment::qualified_ident(&self.schema, &self.table));

        // WHERE
        if !self.conditions.is_empty() {
            frag = frag.push_raw(" WHERE ");
            for (i, wc) in self.conditions.iter().enumerate() {
                if i > 0 {
                    frag = frag.push_raw(" AND ");
                }
                frag = frag
                    .append(SqlFragment::ident(&wc.column))
                    .push_raw(" = ")
                    .append(SqlFragment::param(wc.value.clone()));
            }
        }

        // RETURNING
        frag = frag.push_raw(" RETURNING ");
        if self.returning.is_empty() {
            frag = frag.push_raw("*");
        } else {
            for (i, col) in self.returning.iter().enumerate() {
                if i > 0 {
                    frag = frag.push_raw(", ");
                }
                frag = frag.append(SqlFragment::ident(col));
            }
        }

        frag.build()
    }
}

#[async_trait::async_trait]
impl ExecutableStep for PgDeleteStep {
    fn id(&self) -> StepId {
        self.id
    }

    fn dependencies(&self) -> &[StepId] {
        &self.deps
    }

    fn fingerprint(&self) -> StepFingerprint {
        StepFingerprint::new(
            TypeId::of::<PgDeleteStep>(),
            self.deps.clone(),
            self.compute_config_hash(),
        )
    }

    async fn execute(
        &self,
        _ctx: &ExecutionContext,
        _inputs: StepInputs,
    ) -> Result<StepOutput, FwGraphError> {
        if self.conditions.is_empty() {
            return Err(FwGraphError::ExecutionError(
                "PgDeleteStep: DELETE without WHERE conditions is not allowed".into(),
            ));
        }

        let (sql, sql_params) = self.build_sql();

        debug!(
            step_id = self.id,
            sql = %sql,
            param_count = sql_params.len(),
            "PgDeleteStep executing query"
        );

        let rows = codec::execute_select(&self.pool, &sql, &sql_params)
            .await
            .map_err(|e| FwGraphError::ExecutionError(format!("PgDeleteStep: {}", e)))?;

        let values: Vec<Arc<dyn std::any::Any + Send + Sync>> = rows
            .into_iter()
            .map(|row| Arc::new(row) as Arc<dyn std::any::Any + Send + Sync>)
            .collect();

        Ok(StepOutput::from_values(values))
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    fn mock_pool() -> PgPool {
        sqlx::postgres::PgPoolOptions::new()
            .max_connections(1)
            .connect_lazy("postgres://localhost/test_does_not_exist")
            .expect("connect_lazy should not fail")
    }

    #[tokio::test]
    async fn delete_sql_generation() {
        let step = PgDeleteStep::new(1, mock_pool(), "public", "users")
            .where_eq("id", PgValue::Uuid(uuid::Uuid::nil()))
            .returning(vec!["id".into(), "name".into()]);

        let (sql, params) = step.build_sql();
        assert_eq!(
            sql,
            "DELETE FROM \"public\".\"users\" \
             WHERE \"id\" = $1 \
             RETURNING \"id\", \"name\""
        );
        assert_eq!(params.len(), 1);
    }

    #[tokio::test]
    async fn delete_multiple_conditions() {
        let step = PgDeleteStep::new(1, mock_pool(), "public", "posts")
            .where_eq("author_id", PgValue::Int(42))
            .where_eq("published", PgValue::Bool(false));

        let (sql, params) = step.build_sql();
        assert!(sql.contains("WHERE \"author_id\" = $1 AND \"published\" = $2"));
        assert_eq!(params.len(), 2);
    }

    #[tokio::test]
    async fn delete_returning_star_when_empty() {
        let step = PgDeleteStep::new(1, mock_pool(), "public", "users")
            .where_eq("id", PgValue::Int(1));

        let (sql, _) = step.build_sql();
        assert!(sql.ends_with("RETURNING *"));
    }

    #[tokio::test]
    async fn delete_fingerprint_same_config() {
        let pool = mock_pool();
        let s1 = PgDeleteStep::new(1, pool.clone(), "public", "users")
            .where_eq("id", PgValue::Int(1));
        let s2 = PgDeleteStep::new(2, pool, "public", "users")
            .where_eq("id", PgValue::Int(99));
        assert_eq!(s1.fingerprint(), s2.fingerprint());
    }
}
