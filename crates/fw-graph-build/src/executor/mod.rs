...
use fw_graph_types::{JwtClaims, PgRow, PgValue};
...
impl QueryExecutor {
    ...
    pub async fn insert(
        ...
        use fw_graph_sql::InsertBuilder;
    ...
    pub async fn update(
        ...
        use fw_graph_sql::UpdateBuilder;
    ...
    pub async fn delete(
        ...
        use fw_graph_sql::DeleteBuilder;
        let (pk_cols, pk_vals): (Vec<&str>, Vec<PgValue>) = pk_values.into_iter().unzip();
...
CROSS JOIN LATERAL (
  ...
  LIMIT $2
        ...
        let sqlx_rows = sqlx::query(&sql)
            ...
            .await
            .map_err(QueryError::Database)?;
        let rows: Vec<PgRow> = sqlx_rows.iter().map(fw_graph_dataplan::decode_row).collect();
